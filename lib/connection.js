...
var connection = function(request, socket, head){
  ...
  if (this.webSocketExt == 'x-webkit-deflate-frame'){
    this.browser = 'Chrome';
  }else if (request.headers['user-agent'].match(/Firefox/)){
  ...
  if (this.webSocketVer == '13'){
    console.log('[UPGRADE] browser: ' + this.browser);
    this.emit('handshake');
  }else{
